...
void AVL<T>::makeDeletion(Node<T> *&node)
{
   ...
   else
   {
	  ...
	  if (getBalance(attachPoint) == -2)
      {
         if (getBalance(attachPoint->mLeft) == -1)
            rotateRight(oneAbove->mLeft);
         else
            rotateLeftRight(oneAbove->mLeft);
      }
...
/*      Pre:  A search key
 *     Post:  The word matching the search key
 *  Purpose:  To search for a search key in the AVL and return the matching word using
 *            the typical binary tree search
 **************************************************************/
template <typename T>
T AVL<T>::search(string searchKey)
{
	T data;
	Node<T>* tmp = mRootNode;
